...
class BookingRepository extends BaseRepository
{
    ...
    public function storeJobEmail(array $data)
    {
        ...
        $this->updateJobDetails($job, $user, $data);
    ...
    /**
     * Update job details based on input data.
     *
     * @param Job $job
     * @param User $user
     * @param array $data
     * @return void
     */
    private function updateJobDetails(Job $job, User $user, array $data)
    {
     ...
     * @return void
     */
    private function sendJobEmailNotification(Job $job, User $user)
    {
    ...
    public function jobToData($job)
    {
        $data = [
            ...
            'job_for' => $this->getJobForCategories($job)
        ];
    ...
    /**
     * Get job_for categories based on job's gender and certified type.
     *
     * @param Job $job
     * @return array
     */
    private function getJobForCategories(Job $job)
    {
        $jobFor = [];
    ...
    public function jobEnd($post_data = [])
    {
        ...
        $this->sendSessionEmail($email, $name, $job->id, 'faktura', $data);

        $job->save();

        $translatorJob = $job->translatorJobRel->where('completed_at', null)->where('cancel_at', null)->first();

        $this->fireSessionEndedEvent($job, $post_data['userid'], $translatorJob->user_id);
        ...
        $this->sendSessionEmail($translatorEmail, $translatorName, $job->id, 'lön', $data);
     ...
     * @param array $data
     */
    private function sendSessionEmail($email, $name, $jobId, $forText, $data)
    {
        $subject = "Information om avslutad tolkning för bokningsnummer #{$jobId}";
     ...
     */
    private function fireSessionEndedEvent(Job $job, $userId, $translatorId)
    {
        Event::fire(new SessionEnded($job, ($userId == $job->user_id) ? $translatorId : $job->user_id));
    }
    ...
    public function getPotentialJobIdsWithUserId($userId)
    {
        ...
        $jobType = $this->getJobTypeForTranslator($translatorType);
        $userLanguage = UserLanguages::where('user_id', '=', $userId)->pluck('lang_id')->all();
        ...
        $jobIds = $this->filterJobIdsByTown($jobIds, $userId);

        return TeHelper::convertJobIdsInObjs($jobIds);
    }

    private function getJobTypeForTranslator($translatorType)
    {
        return [
            'professional' => 'paid',
            'rwstranslator' => 'rws',
            'volunteer' => 'unpaid',
        ][$translatorType] ?? 'unpaid';
    }

    private function filterJobIdsByTown($jobIds, $userId)
    {
        return array_filter($jobIds, function ($jobId) use ($userId) {
            $job = Job::find($jobId);
    ...
    public function sendNotificationTranslator($job, $data = [], $excludeUserId)
    {
            ...
            foreach ($jobs as $potentialJob) {
                if ($job->id == $potentialJob->id) {
                    ...
                    if ($jobForTranslator == 'SpecificJob' && $this->isJobEligibleForTranslator($userId, $potentialJob)) {
                        $translatorsList = $this->isNeedToDelayPush($user->id) ? $delayedPushTranslators : $suitableTranslators;
        ...
        $msgContents = $this->buildNotificationMessage($data);

        $msgText = ["en" => $msgContents];

        $this->logPushInfo($job->id, $suitableTranslators, $delayedPushTranslators, $msgText, $data);
        $this->sendPushNotificationToSpecificUsers($suitableTranslators, $job->id, $data, $msgText, false);
        $this->sendPushNotificationToSpecificUsers($delayedPushTranslators, $job->id, $data, $msgText, true);
    }

    private function isUserTranslatorEligible($user, $data)
    {
        return $user->user_type == '2' && $user->status == '1' && $this->isNeedToSendPush($user->id) &&
            !($data['immediate'] == 'yes' && TeHelper::getUsermeta($user->id, 'not_get_emergency') == 'yes');
    }

    private function isJobEligibleForTranslator($userId, $potentialJob)
    {
    ...
    private function buildNotificationMessage($data)
    {
    ...
    private function logPushInfo($jobId, $suitableTranslators, $delayedPushTranslators, $msgText, $data)
    {
        $logger = new Logger('push_logger');
        $logger->pushHandler(new StreamHandler(storage_path("logs/push/laravel-" . date('Y-m-d') . ".log"), Logger::DEBUG));
        $logger->pushHandler(new FirePHPHandler());
        $logger->addInfo("Push send for job {$jobId}", [$suitableTranslators, $delayedPushTranslators, $msgText, $data]);
    }
    /**
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        $message = $this->chooseSMSMessage($job->customer_physical_type, $job->customer_phone_type, $phoneJobMessageTemplate, $physicalJobMessageTemplate);
    ...
    private function chooseSMSMessage($physicalType, $phoneType, $phoneTemplate, $physicalTemplate)
    {
        if ($physicalType == 'yes' && $phoneType == 'no') {
            ...
            return ''; // Shouldn't be feasible, so no handling of this edge case
        }
    }

    private function sendSMSToTranslators($translators, $message)
    {
    ...
    public function sendPushNotificationToSpecificUsers($users, $jobId, $data, $msgText, $isNeedDelay)
    {
        ...
        $this->setNotificationSounds($data, $iosSound, $androidSound);
    ...
    private function setNotificationSounds($data, &$iosSound, &$androidSound)
    {
    ...
    private function getTranslatorLevel($job)
    {
        $translator_level = [];
    ...
    public function updateJob($id, $data, $cuser)
    {
        ...
        $logData = [
            $this->changeTranslator($currentTranslator, $data, $job),
            $this->changeDue($job, $data['due']),
            $this->changeLanguage($job, $data['from_language_id']),
            $this->changeStatus($job, $data),
        ];
        ...
        if ($job->due <= Carbon::now()) {
            ...
            $this->handleNotifications($job, $currentTranslator, $data, $logData);
        }
    }
    ...
    private function handleNotifications(Job $job, $currentTranslator, $data, $logData)
    {
        $this->sendNotifications($job, $currentTranslator, $data, $logData['1']['oldTime']);
        $this->sendChangedLangNotification($job, $logData['2']['oldLang']);
    }

    private function sendNotifications(Job $job, $currentTranslator, $data, $oldTime)
    {
        if ($logData['1']['dateChanged']) {
            $this->sendChangedDateNotification($job, $oldTime);
        }

        $changeTranslator = $logData['0'];
    ...
    private function changeLanguage(Job $job, $newLang)
    {
        $logData = [];
        if ($job->from_language_id != $newLang) {
            $logData = [
                'oldLang' => TeHelper::fetchLanguageFromJobId($job->from_language_id),
                'newLang' => TeHelper::fetchLanguageFromJobId($newLang),
            ];
            $job->from_language_id = $newLang;
        }
        return $logData;
    }
